...
```bash
> glideetl export_blocks_and_transactions -h

Usage: glideetl export_blocks_and_transactions [OPTIONS]
...
```bash
> glideetl extract_csv_column --input transactions.csv --column hash --output transaction_hashes.txt
```

Then export receipts and logs:

```bash
> glideetl export_receipts_and_logs --transaction-hashes transaction_hashes.txt \
...
```bash
> glideetl extract_token_transfers --logs logs.csv --output token_transfers.csv
```
...
```bash
> glideetl extract_csv_column --input receipts.csv --column contract_address --output contract_addresses.txt
```

Then export contracts:

```bash
> glideetl export_contracts --contract-addresses contract_addresses.txt \
...
```bash
> glideetl filter_items -i contracts.json -p "item['is_erc20'] or item['is_erc721']" | \
glideetl extract_field -f address -o token_addresses.txt
```

Then export ERC20 / ERC721 tokens:

```bash
> glideetl export_tokens --token-addresses token_addresses.txt \
...
```bash
> glideetl export_geth_traces --start-block 0 --end-block 500000 \
...
```bash
> glideetl extract_geth_traces --input geth_traces.json --output traces.csv
```
...
```bash
> glideetl get_block_range_for_date --provider-uri=https://mainnet.infura.io/v3/7aef3f0cd1f64408b163814b22cc643c --date 2018-01-01
4832686,4838611
```
...
```bash
> glideetl get_keccak_hash -i "transfer(address,uint256)"
...
> glideetl stream --provider-uri https://mainnet.infura.io/v3/7aef3f0cd1f64408b163814b22cc643c --start-block 500000
```
...
> glideetl stream --start-block 500000 --output projects/<your-project>/topics/crypto_ethereum
```

Stream blockchain data to a Postgres database:

```bash
glideetl stream --start-block 500000 --output postgresql+pg8000://<user>:<password>@<host>:5432/<database>
```
